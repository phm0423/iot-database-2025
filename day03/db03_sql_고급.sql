...
-- 4-2 4.875를 소수점 첫번재 자리까지 반올림하시오.
SELECT ROUND(4.875, 1) AS 결과;

-- 4-3 고객별 평균 주문 금액을 100원 단위로 반올림하시오.
...
SELECT bookname AS '제목'
	 , CHAR_LENGTH(bookname) AS '문자수' -- 글자 길이를 구할때
     , LENGTH(bookname) AS '바이트수'	 -- utf8에서 한글 한글자의 바이트수는 
  FROM Book
...
select SUBSTR('Hello MySQL', 1, 1); -- 연습이 필요
...
-- %Y = 년도 전체, %y = 년도 뒤 2자리, %M = July(월이름), %b = Jul(월약어) %m = 07(월숫자)
-- %d = 일, %H = 16, %h = 04(오후도), %W = Monday, %w = 01(요일은 일요일 0)
-- %p = AM/PM
SELECT orderid as '주문번호'
