...
- MySQL 설치(Docker)
    ...
    5. 컨테이너 확인
     ...
     NAMES
        73032d15fb51   mysql:latest   "docker-entrypoint.s…"   30 seconds ago   Up 30 seconds   0.0.0.0:3306->3306/tcp, 33060/tcp   mysql-container
        ```
...
<img src='./img/db001.png' width = '700'>

- Workbench 다운
    - https://dev.mysql.com/downloads/workbench/
    - MySQL Installer에서 Workbench, Sample만 설치
    - Workbench 실행후
        1. MySQL Connectoins + 클릭
...
- SQL 기초
    ...
    - DML 중 SELECT
        ...
        - 쿼리 연습(정렬까지) : [SQL](./day02/da02_select_쿼리연습.sql)
        - 쿼리 연습(집계함수부터) : [SQL](./day02/da03_select_집계함수부터.sql)
...
- Visual Studio Code에서 MySQL 연동
    ...
    - Database Client
        ...
        3. 정보 입력 > 연결 테스트
            ...
            <img src='./img/db002.png' width = '600'>

        4. Workbench 처럼 사용

            <img src='./img/db003.png' width = '600'>
...
- SQL 기초
    ...
    - DDL 중 CREATE
        ...
        (
            컬럼(속성)명 제약사항들,...
            PRIMARY KEY (컬럼(들))
            FOREIGN KEY (컬름(들)) REFERENCES 테이블명(컬럼(들)) ON 제약사항
        );
        ```
        ...
        - 테이블 생성 후 확인
            ...
            <img src='./img/db004.png' width='600'>
...
- SQL 고급
    ...
    - 인덱스 : [SQL](./day04/db04_sql고급_인덱스.sql)
        ...
        - 주의점
            ...
            - (NULL)값이 많은 컬럼에 인덱스 생성말 것(성능저하)

- 데이터베이스 프로그래밍
    - 저장 프로시저 : [SQL](./day04/db04_저장프로시저1.sql)
        - 너무 많은 쿼리로 일을 처리해야 할때, 파이썬 등 프로그램에서 구현하면 매우 복작함
...
- 정규화 : [SQL](./day06/db03_정규화_이상현상.sql)
    ...
    - 함수 종속성 - 하나의 속성(A)이 다른 속성(B)을 결정지으면 A는 B의 결정자. A → B
        ...
        - BCNF 해당하지 않는 경우 - A → C, B → C 일 경우 C → B 경우가 발생
...
- 트랜잭션 : [SQL](./day06/db05_트랜잭션2.sql)
    ...
    - 특징
        ...
        - C(일관성) - Consistency, 트랜잭션 전후에 데이터가 일관되게 저장되어 있어야 함
...
- 데이터 베이스 관리와 보안 : [SQL](./day07/db02_DB관리.sql)
- 실무실습 : [SQL](./day07/db03_쿼리연습.sql)
    - 서브쿼리까지

## 8일차
- 실무실습 : [SQL](./day08/db01_쿼리연습.sql)
    - 서브쿼리부터
- 데이터모델링 실습
    - 병원 업무 관리 ERD
    ...
    <img src='./img/db006.png' width = '600'>
...
- tkinter DB연동 GUI앱 개발 : [Python](./day09/students_regapp.py)
    ...
    3. tkinter 템플릿코드작성 - 기본적인 GUI앱 틀

        <img src='./img/db007.png' width='700'>
    
    4. 데이터베이스 CRUD 함수 구현
        
        <img src='./img/db008.png' width='700'>
- 데이터베이스 연습
    - SQL, 모델링 연습
- 코딩테스트