...
-- 오류해결실행 ->SET SESSION sql_mode = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
...
/* 문제1 - 이전문제를 활용, 집계결과가 아니면(ALL-DEPTS)라고 출력, 업무에 대한 집계 결과가 아니면 (ALL-JOBS)를 출력
			rollup으로 만들어진 소계면(ALL-JOBS), 총계면(ALL-DEPTS)
*/
select case grouping(department_id) when 1 then '(ALL_DEPTS)' else ifnull(department_id, '부서없음') end as 'Dept#'
     ...
     -- , grouping(job_id)
     , round(avg(salary)*12, 0) as 'Avg Ann_sal'
  from employees
group by department_id, job_id
  with rollup;
...
/* 문제 1 - 부서별 급여를 기준으로 내림차순 정렬하시오. 이때 다음 세가지 함수를 이요하여, 순위를 출력하시오 (107행)
*/
select employee_id
	 , last_name
     , salary
