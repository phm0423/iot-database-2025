...
-- 파이썬, C#등 프로그래밍언어로 가져갈땐, 커럶이름 컬럼갯수를 모두 파악해야 하기 때문에 아래와 같이 사용
...
-- > 미만 < 초가 >= 이하 <= 이상 <> 같지않다 != 같지않다
SELECT *
  FROM Book
 WHERE price < 20000 
   AND publisher = '굿스포츠';
   
-- 3-5 : 가격이 10000원이상 20000원 이하인 도서를 검색하시오.
SELECT *
  FROM Book
...
--       _구% : 첫번째글자는 뭐든지 상관없고, 두번째글자가 구로 되고 뒤에 글이 더 있다.
SELECT *
  FROM Book
