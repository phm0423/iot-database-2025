...
/* 문제1 - employee 에서 사원의 성과 이름은 Name, 업무는 Job, 급여는 Salary, 연봉에 $100 보너스를 추가하여 계산한 값은 Increased Ann_Salary, 
...
/* 문제 2 - Employees 에서 모든 사원의 이름(last_name)과 연봉을 '이름: 1 Year Salary = $연봉' 형식으로 출력하고, 1 Year Salary라는 별칭으로 붙이세요.(107행)
*/
...
-- where, order by
/* 샘플 - hr부서 예산 편성 문제로 급여 정보 보고서를 작성한다. employees에서 salary 가 7000 ~ 10000 달러 범위 이외의 사람의 
...
/* 문제2 - 현재 날짜 타입을 날짜함수를 통해 확인, 1995년 5월 20일 ~ 1996년 5일 20일 사이 입사하나 사원의 이름 Name으로 별칭
...
-- 단일행 함수 및 변환 함수
/* 문제1 - 이름이 s로 끝나는 각 사원의 업무를 아래의 예와 같이 출력하고자 함.(18행)
...
/* 문제1 - 사원이 소속된 부서별 급여 합계, 급여 평균, 급여 최대값, 급여 최소값을 집계.
			출력 값은 여섯 자리와 세 자리 구분 기호, $ 표시 포함, 부서번호 오름차순
            단, 부서에 소속되지 않은 사원은 정보에서 제외, 출력 시 머리 글은 아래처럼 별칭 처리(11행)
*/
...
group by department_id;	-- 그룹바이 속한 컬럼만 select 절에 사용가능
...
-- 서브쿼리
/* 문제3 - 사원들의 지역별 근무현황 조회. 도시 이름이 영문'O로 시작하는 지역에 살고 
			사번, 이름, 업무 입사일 출력(34행)
*/
